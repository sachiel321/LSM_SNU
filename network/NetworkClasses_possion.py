...
class cere_model(nn.Module):
    ...
    def forward(self,m,u,sigma,f, time_window):
        ...
        m_MF_mem = m_MF_spike = u_MF_mem = u_MF_spike = sigma_MF_mem = sigma_MF_spike = f_MF_mem = f_MF_spike = torch.zeros(self.batch_size, self.num_out_MF).to(self.device)
        m_GC_mem = m_GC_spike = u_GC_mem = u_GC_spike = sigma_GC_mem = sigma_GC_spike = f_GC_mem = f_GC_spike = torch.zeros(self.batch_size, self.num_out_GC).to(self.device)
        m_PCE_mem = m_PCE_spike = u_PCE_mem = u_PCE_spike = m_PCI_mem = m_PCI_spike = u_PCI_mem = u_PCI_spike =  torch.zeros(self.batch_size, self.num_out_PC).to(self.device)
        m_DCN_mem = m_DCN_spike = torch.zeros(self.batch_size, self.num_out_DCN*8).to(self.device)
        ...
        if type(m) is np.ndarray:
            ...
            sigma = sigma.float().to(self.device)
        ...
        for step in range(time_window):
            ...
            for t in range(time_window):
                ...
                with torch.no_grad():
                    ...
                    self.monitor_DCN_m[:,:,t] = m_DCN_spike.detach()
...
class prefrontal_model(nn.Module):
    def __init__(self, batch_size, num_hidden1, num_hidden2, num_hidden3,N_step,possion_num=50,gpu='0'):
        ...
        self.layer3_in = torch.zeros(1,self.inputnum3).to(self.device)

        #monitor
        self.monitor_h11 = torch.zeros(self.batch_size, self.num_hidden1, self.possion_num).to(self.device)
        self.monitor_h12 = torch.zeros(self.batch_size, self.output1, self.possion_num).to(self.device)
        self.monitor_h21 = torch.zeros(self.batch_size, self.num_hidden2, self.possion_num).to(self.device)
        self.monitor_h22 = torch.zeros(self.batch_size, self.output2, self.possion_num).to(self.device)
        self.monitor_layer3_in = torch.zeros(self.batch_size, self.inputnum3, self.possion_num).to(self.device)
        self.monitor_h31 = torch.zeros(self.batch_size, self.num_hidden3, self.possion_num).to(self.device)
        self.monitor_h31_5 = torch.zeros(self.batch_size, self.num_hidden3, self.possion_num).to(self.device)
        self.monitor_h32 = torch.zeros(self.batch_size, self.output3, self.possion_num).to(self.device)
    ...
    def forward(self,input11,input12,input2,input3,input4, time_window):
        ...
        h11_mem = h11_spike = h11_sumspike = torch.zeros(self.batch_size, self.num_hidden1).to(self.device)
        h12_mem = h12_spike = h12_sumspike = torch.zeros(self.batch_size, self.output1).to(self.device)
        h21_mem = h21_spike = h21_sumspike = torch.zeros(self.batch_size, self.num_hidden2).to(self.device)
        h22_mem = h22_spike = h22_sumspike = torch.zeros(self.batch_size, self.output2).to(self.device)
        h31_mem = h31_spike = h31_sumspike = torch.zeros(self.batch_size, self.num_hidden3).to(self.device)
        h31_5_mem = h31_5_spike = h31_5_sumspike = torch.zeros(self.batch_size, self.num_hidden3).to(self.device)
        h32_mem = h32_spike = h32_sumspike = torch.zeros(self.batch_size, self.output3).to(self.device)
    
        if type(input11) is np.ndarray:
                input11 = torch.from_numpy(input11).float().to(self.device)
                input12 = torch.from_numpy(input12).float().to(self.device)
                input2 = torch.from_numpy(input2).float().to(self.device)
                input3 = torch.from_numpy(input3).float().to(self.device)
                input4 = torch.from_numpy(input4).float().to(self.device)
        else:
            input11 = input11.float().to(self.device)
            input12 = input12.float().to(self.device)
            input2 = input2.float().to(self.device)
            input3 = input3.float().to(self.device)
            input4 = input4.float().to(self.device)
        ...
        for step in range(time_window):
            ...
            for t in range(time_window):
                ...
                with torch.no_grad():
                    self.monitor_h11[:,:,t] = h11_spike.detach()
                    self.monitor_h12[:,:,t] = h12_spike.detach()
                    self.monitor_h21[:,:,t] = h21_spike.detach()
                    self.monitor_h22[:,:,t] = h22_spike.detach()
                    self.monitor_layer3_in[:,:,t] = self.layer3_in.detach()
                    self.monitor_h31[:,:,t] = h31_spike.detach()
                    self.monitor_h31_5[:,:,t] = h31_5_spike.detach()
                    self.monitor_h32[:,:,t] = h32_spike.detach()
    ...
    def stdp_step(self, reward,lr):
        ...
        r_stdp(self.monitor_h31_5[0],self.monitor_h32[0],self.fc32.weight, reward,lr=lr)
        r_stdp(self.monitor_h31[0],self.monitor_h31_5[0],self.fc31_5.weight, reward,lr=lr)
