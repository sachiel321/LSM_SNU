...
class simulate:
    ...
    def step(self,action):
        ...
        if abs(self.f_x) >= 500 or abs(self.f_y) >= 500 or self.pa.getz() < -5 or self.pa.getz() > 55 * 1000: #dangerous detection
            self.isbottom = -1

        elif self.pa.getz() >= self.pb.getz(): #bottom detection
            self.isbottom = 1  

        return np.array([self.f_x, self.f_y, self.pa.getz(),delt_z+noise_z]).astype(np.float32), self.isbottom
    ...
    def reset(self):
        ...
        self.f_y = 0
        self.isbottom = 0
        return np.array([self.f_x, self.f_y, self.pa.getz(),0]).astype(np.float64)
